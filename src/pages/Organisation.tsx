...
const typeColors: Record<string, any> = {
  SERVICE: 'primary', DIRECTION: 'error',
  DEPARTEMENT: 'warning', POSTE: 'default'
};
...
const Organisation = () => {
  ...
  const [form, setForm] = useState({
    nom: '', description: '', type: 'SERVICE', parentId: '', ordre: 0
  });
  ...
  const openEditDialog = (n: any) => {
    setSelected(n);
    setForm({
      nom: n.nom, description: n.description || '',
      type: n.type, parentId: n.parentId || '', ordre: n.ordre
    });
    setOpenEdit(true);
  };
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2}>
        {noeuds.map((n) => (
          <Grid item xs={12} sm={6} md={4} key={n.id}>
            <Card sx={{ borderRadius: 3, borderLeft: '4px solid #5B21B6' }}>
              <CardContent sx={{ p: 2.5 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <AccountTree sx={{ color: '#5B21B6', fontSize: 20 }} />
                    <Box>
                      <Typography sx={{ fontWeight: 600, fontSize: 14 }}>{n.nom}</Typography>
                      {n.parentNom && (
                ...
                <Box sx={{ display: 'flex', gap: 0.5, mt: 1.5, justifyContent: 'flex-end' }}>
                  ...
                  <Tooltip title="Désactiver">
                    <IconButton size="small"
                      onClick={() => { setSelected(n); setOpenDelete(true); }}>
      ...
      <Dialog open={openAdd} onClose={() => setOpenAdd(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 600, fontSize: 15 }}>➕ Nouveau noeud</DialogTitle>
        <DialogContent>
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setOpenAdd(false)} sx={{ color: '#64748b' }}>Annuler</Button>
      ...
      <Dialog open={openEdit} onClose={() => setOpenEdit(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 600, fontSize: 15 }}>✏️ Modifier</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <TextField fullWidth size="small" label="Nom"
              value={form.nom}
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setOpenEdit(false)} sx={{ color: '#64748b' }}>Annuler</Button>
          <Button variant="contained" onClick={handleEdit}
            sx={{ borderRadius: 2, background: '#5B21B6' }}>Enregistrer</Button>
        </DialogActions>
      </Dialog>

      {/* Dialog Supprimer */}
      <Dialog open={openDelete} onClose={() => setOpenDelete(false)}>
        <DialogTitle sx={{ fontWeight: 600, fontSize: 15 }}>⚠️ Désactiver</DialogTitle>
        <DialogContent>
          <Alert severity="warning" sx={{ mb: 1 }}>
            Cette action désactivera le noeud.
          </Alert>
          <Typography>
            Désactiver <strong>{selected?.nom}</strong> ?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setOpenDelete(false)} sx={{ color: '#64748b' }}>Annuler</Button>
          <Button variant="contained" color="error" onClick={handleDelete}
            sx={{ borderRadius: 2 }}>Confirmer</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
